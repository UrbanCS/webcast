...
$locked = !empty($requiresAccessCode) && empty($accessGranted);
?>
<article class="broadcast-page">
    <header class="broadcast-hero">
        <div class="broadcast-hero__content">
            ...
            <h1><?= e($event['title']); ?></h1>
            <?php if (!$locked && !empty($event['description'])): ?>
                <p class="lede"><?= nl2br(e((string) $event['description'])); ?></p>
            <?php endif; ?>
            <?php if ($locked): ?>
                <p class="lede"><?= e(lang('access_required_message')); ?></p>
            <?php else: ?>
                <div class="meta-row">
                    <div class="meta-chip">
    ...
    <?php if ($locked): ?>
        <section class="section-block access-gate">
            <div class="section-block__header">
                <div>
                    <p class="eyebrow"><?= e(lang('private_access')); ?></p>
                    <h2><?= e(lang('access_required')); ?></h2>
                </div>
            </div>

            <?php if (!empty($accessErrors['code'])): ?>
                <div class="alert alert-error"><?= e($accessErrors['code']); ?></div>
            <?php endif; ?>

            <form class="access-form" method="post" action="<?= e(current_url()); ?>">
                <input type="hidden" name="_csrf" value="<?= e(csrf_token('event_access_' . $event['id'])); ?>">
                <label class="form-field">
                    <span><?= e(lang('access_code')); ?></span>
                    <input type="password" name="access_code" required autocomplete="one-time-code">
                </label>
                <button class="button button-primary" type="submit"><?= e(lang('unlock_event')); ?></button>
            </form>
        </section>
    <?php else: ?>
